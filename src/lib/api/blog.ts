...
export const getBlogStats = async (): Promise<BlogStats> => {
  const response = await apiClient.get(`/api/v1/blogs/admin/stats`);
  return response.data;
};
...
export const updateBlog = async (
  id: string,
  data: FormData | Partial<BlogFormData>
): Promise<Blog> => {
  const response = await apiClient.put(`/api/v1/blogs/edit/${id}`, data);
  return response.data;
};

export const clearBlogComment = async (
  id: string
): Promise<{ message: string; blog: Blog }> => {
  const response = await apiClient.patch(`/api/v1/blogs/${id}/comment/clear`);
  return response.data;
};
...
export const updateBlogStatus = async (
  ...
  const response = await apiClient.patch(`/api/v1/blogs/${id}/status`, {
    status,
    comment,
  });
  return response.data;
