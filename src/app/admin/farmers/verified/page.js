...
export default function UserManagement() {
  ...
  useEffect(() => {
    ...
    // Always filter for verified users only on this page
    ...
    // Filter by verification status (keeping for UI consistency, but only verified shown)
    if (filters.status) {
      if (filters.status === "complete") {
        filtered = filtered.filter((user) => user.farmProfile);
